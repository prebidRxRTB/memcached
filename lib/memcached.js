...
Client.config = {
    ...
    , reqTimeout: 1000,      // request timeout
};

const maxRequestID = 1<<30;

let reqId =  maxRequestID,
    cmd   = {};
function getRequestId() {
    if (reqId > maxRequestID) reqId = 0;
    return reqId++;
}
...
(function (nMemcached) {
        ...
        manager.factory(function factory() {
            var S = Array.isArray(serverTokens)
                ...
                if(S.metaData.length) {
                    S.metaData.forEach(item => {
                        if (item && item.callback){
                            item.execution = Date.now() - item.start;
                            memcached.delegateCallback(item, e, false, item.callback);
                        }
                    });
    ...
    memcached.command = function memcachedCommand(queryCompiler, server) {
        ...
        this.connect(server, function allocateMemcachedConnection(error, S) {
            ...
            // request timeout
            query.reqId = getRequestId();
            cmd[query.reqId] = setTimeout(() => {
                if(cmd[query.reqId]) {
                    delete cmd[query.reqId];
                    memcached.makeCallback(query.callback, new Error('memcached request timeout, t: ' + memcached.reqTimeout));
                }
            }, memcached.reqTimeout);
    ...
    memcached.delegateCallback = function(){
        ...
        if(cmd[master.reqId]) {
            clearInterval(cmd[master.reqId]);

            delete cmd[master.reqId];
